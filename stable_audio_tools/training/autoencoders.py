...
class AutoencoderTrainingWrapper(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # encoder_input = torch.abs(encoder_input)

        # import matplotlib.pyplot as plt

        # # Plot the encoder input for one channel and one waveform
        # plt.figure(figsize=(10, 4))
        # plt.plot(encoder_input[0, 0, :].cpu().detach().numpy())
        # plt.title('Encoder Input - Channel 1, Waveform 1')
        # plt.xlabel('Samples')
